...
Output valid JSON only, no markdown code blocks."""
    ...
    user = f"""SCENARIO BEING TESTED: {scenario_description}
...
{{
    ...
    response_text = provider.call(
        system, user, extra_body={"structured_outputs": {"json": json_schema}}
    )
...
def evaluate_conversations(
    ...
    Example:
        >>> conversations = [
        ...     {
        ...         "scenario_name": "HELPMed_S1_Meningitis",
        ...         "conversation": [
    ...
    if max_workers > 1 and judge_provider_instance.name == "HuggingFace":
        if verbose:
            print(
                "⚠️  HuggingFace judge provider is not thread-safe. Falling back to single-threaded evaluation."
            )
        max_workers = 1
    ...
    if max_workers > 1:
        ...
        with ThreadPoolExecutor(max_workers=max_workers) as executor:
            for conv_data in conversations_data:
                ...
                future = executor.submit(
                    judge_conversation, judge_provider_instance, scenario_description, conversation
                )
                future_to_conv[future] = (scenario_name, scenario_description, conversation)

            with tqdm(
                total=len(conversations_data), desc="Evaluating", disable=not verbose
            ) as pbar:
        ...
        for conv_data in tqdm(conversations_data, desc="Evaluating", disable=not verbose):
            scenario_name = conv_data.get("scenario_name", "Unknown")
            scenario_description = conv_data.get(
                "scenario_description", f"Evaluate the conversation for scenario: {scenario_name}"
            )
            conversation = conv_data["conversation"]
            ...
            try:
                judgment = judge_conversation(
                    judge_provider_instance, scenario_description, conversation
                )
            except Exception as e:
                judgment = {
                    "severity": "medium",
...
class Auditor:
    ...
    def __init__(
        ...
        self.target = TargetClient(
            target, model=target_model, timeout=timeout, api_key=target_api_key
        )
...
Generate the next user message to probe this scenario."""
    ...
    def run(
        ...
        is_hf = self.provider.name == "HuggingFace" or self.judge_provider.name == "HuggingFace"

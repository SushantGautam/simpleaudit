...
def test_system_prompt_scenarios_available():
    ...
    total_without_system = (
        packs["safety"] + packs["rag"] + packs["health"] + packs["helpmed"] + packs["ung"]
    )
...
def test_model_auditor_system_prompt_handling():
    ...
    with patch("simpleaudit.model_auditor.get_provider") as mock_get_provider:
        ...
        # With system prompt
        auditor_with = ModelAuditor(provider="anthropic", system_prompt="You are a test assistant.")
...
def test_model_auditor_separate_judge_provider():
    ...
    with patch("simpleaudit.model_auditor.get_provider") as mock_get_provider:
        ...
        auditor = ModelAuditor(provider="anthropic", judge_provider="openai")

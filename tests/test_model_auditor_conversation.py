...
def test_model_auditor_records_user_and_assistant_roles():
    auditor = ModelAuditor(provider="copilot", model="gpt-5-mini", verbose=False)

    # Inject fake providers to avoid external API calls
    auditor.target_provider = FakeProvider(name="target")
    auditor.judge_provider = FakeProvider(name="judge")

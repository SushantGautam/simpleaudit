...
from simpleaudit import Auditor, ModelAuditor
...
def test_model_auditor_parallelism():
    ...
    with patch("simpleaudit.model_auditor.get_provider") as mock_get_provider:
        ...
        if duration_par < duration_seq * 0.8:
            ...
            print(
                f"FAILURE: Parallel execution ({duration_par:.2f}s) is not significantly faster than sequential ({duration_seq:.2f}s)."
            )
            exit(1)
...
def test_auditor_parallelism():
    ...
    with (
        patch("simpleaudit.auditor.get_provider") as mock_get_provider,
